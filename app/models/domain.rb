class Domain < ActiveRecord::Base
  ...
  scope :renewals, order('renewal_date DESC')#, where(:id => Renewal.hosting_id)

  # validates :contact_email, :format => { :with => /^([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4})?$/, :message => 'must be valid email.' }

  PACKAGE_TYPE = ["Trilby Small", "Trilby Medium", "Trilby Large", "Trilby XL", "Legacy Email", "Domain Only"]
end